...
SYSTEM_PROMPT = (
    ...
    "your `answer` to <path> after the user approves it. Take <path> from the intent "
    "(e.g. `→ ~/Documents/server.js`, or 'in ~/Documents'); if no path is given, "
    "choose a sensible name under ~/Documents and say which in the answer. Do NOT put "
    "the file body in the command and do NOT use a heredoc — only `dwim-write <path>`.\n"
