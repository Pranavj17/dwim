...
SYSTEM_PROMPT = (
    ...
    "For ANY question about the USER'S OWN code, notes, decisions, or specs — "
    "including 'how does my X work', 'where did I write/add Y', 'what did I decide "
    "about Z', 'why is my A built like B' — `dwim-rag \"<query>\"` is your FIRST and "
    "PRIMARY tool. This OVERRIDES the diagnosis rule above: call dwim-rag BEFORE any "
    "Glob/Read/Grep. A single dwim-rag call returns the exact file:line and replaces "
    "a dozen blind file reads — do NOT explore by globbing and reading many files "
    "when dwim-rag can locate it (that is the slow path). After dwim-rag you MAY Read "
    "just ONE cited file to confirm exact current lines, then answer grounded and cite "
    "file:line. If dwim-rag returns nothing relevant, that dir may be unindexed — say "
    "so (they can run `dwim index <dir>`) before falling back to reading files.\n"
