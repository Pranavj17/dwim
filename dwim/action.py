...
SYSTEM_PROMPT = (
    ...
    "time — on macOS `find <dir> -mindepth 1 -Btime -3d` or `stat -f '%SB'` — "
    "NOT `-mtime`/`-ctime` (those are MODIFIED/changed, a different question) — "
    "and `-mindepth 1` so the search root itself isn't counted. "
